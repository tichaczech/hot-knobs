...
export default function SignInPage({ continueWithUrl }: SignInSearch) {
    const { signInWithEmail, signInWithFacebook, signInWithGoogle } = useAuth();
    const { t } = useTranslation();

    const handleSignInWithEmail = async (values: SignIn) => {
            ...
            <div className="flex w-full max-w-sm flex-col gap-6">
                <Card>
                    <CardHeader className="text-center justify-center">
                        ...
                        <CardDescription>
                            {t("auth.signIn.page.socialSignInDescription")}
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="grid gap-6">
                            <div className="flex flex-col gap-4">
                                <Button type="button" variant="outline" className="w-full" disabled>
                                    ...
                                    {t("auth.signIn.page.socialSignInApple")}
                                </Button>
                                <Button type="button" variant="outline" className="w-full" onClick={handleSignInWithFacebook}>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#1877F2">
                                        <path
                                            d="M13.764 23.625V13.375h3.478l.52-4.037h-3.998V6.79c0-1.168.324-1.963 1.998-1.963h2.135V1.191A28.347 28.347 0 0014.853 1C11.732 1 9.711 2.83 9.711 6.383v2.955H6.375v4.037h3.336v10.25h4.053z"
                                            fill="currentColor"
                                        />
                                    </svg>
                                    {t("auth.signIn.page.socialSignInFacebook")}
                                </Button>
                                <Button type="button" variant="outline" className="w-full" onClick={handleSignInWithGoogle}>
                                    ...
                                    {t("auth.signIn.page.socialSignInGoogle")}
                                </Button>
                                <Button type="button" variant="outline" className="w-full" disabled>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                    ...
                                    {t("auth.signIn.page.socialSignInMicrosoft")}
                                </Button>
                            </div>
                            <div className="relative text-center text-sm after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t after:border-border">
                                <span className="relative z-10 bg-background px-2 text-muted-foreground">
                                    {t("auth.signIn.page.emailSignInDescription")}
                                </span>
                            </div>
                        </div>
                        <br /> {/* TODO: Use CSS for vertical separtion instead of HTML. */}
                        <SignInForm onSignInWithEmail={handleSignInWithEmail} />
                        <br /> {/* TODO: Use CSS for vertical separtion instead of HTML. */}
                        <div className="text-center text-sm">
                            {t("auth.signIn.page.noAccountPrompt")}{" "}
                            <Link to='/signup' search={{ continueWithUrl }} className="underline underline-offset-4">
                                {t("auth.signIn.page.signUpLink")}
                            </Link>
                        </div>
                    </CardContent>
                </Card>
                <div className="text-balance text-center text-xs text-muted-foreground [&_a]:underline [&_a]:underline-offset-4 [&_a]:hover:text-primary  ">
                    <Trans i18nKey="auth.signIn.page.termsAndPolicyPrompt">
