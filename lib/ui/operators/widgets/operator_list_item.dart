...
class OperatorListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isActive = operator.active;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _MetaRow(operator: operator, surface: surface),
                  ],
                ),
              ),
            ],
...
class _Avatar extends StatelessWidget {
  const _Avatar({required this.operator});
  final Operator operator;
  ...
  Widget build(BuildContext context) {
    final placeholder = CircleAvatar(
      ...
      child: Text(
        operator.name.isNotEmpty ? operator.name.substring(0, 1).toUpperCase() : '?',
        style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold),
      ),
    );

    if (operator.logoUrl == null || operator.logoUrl!.isEmpty) {
      return placeholder;
    }
    ...
    return CircleAvatar(
      ...
      backgroundImage: NetworkImage(operator.logoUrl!),
...
class _StatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (bg, fg, label) = isActive
        ...
        : (colorScheme.surfaceContainerHighest, colorScheme.onSurfaceVariant, 'Inactive'); // TODO: localize
    return Container(
...
class _Menu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        const PopupMenuItem(value: 'edit', child: ListTile(leading: Icon(Icons.edit_outlined), title: Text('Edit'))), // TODO: localize
        const PopupMenuItem(
          value: 'delete',
...
class _MetaRow extends StatelessWidget {
  const _MetaRow({required this.operator, required this.surface});
  final Operator operator;
  final Color surface;
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      runSpacing: 4,
      children: [
        _MetaIconText(icon: Icons.badge_outlined, text: operator.id, style: style),
        _MetaIconText(icon: Icons.schedule_outlined, text: _formatDate(operator.createdAt), style: style),
        if (operator.updatedAt.isAfter(operator.createdAt))
          _MetaIconText(icon: Icons.update, text: _formatDate(operator.updatedAt), style: style),
      ],
    );
  }
