...
public class DeviceUseCases : EntityUseCases<Device, DeviceCreateModel, DeviceUpdateModel>, IDeviceUseCases
{
	...
	/// <inheritdoc />
	public IAsyncEnumerable<string> ListAsync(string? query = default, string? profileId = null, DateTimeOffset? modifiedSince = default, bool onlyActive = true, CancellationToken cancellationToken = default)
	{
		Expression<Func<Device, bool>>? where = device => true;
		if (!String.IsNullOrEmpty(query))
			where = where.AndAlso(device => device.Name.ToLower().Contains(query.ToLower()));
		if (!String.IsNullOrEmpty(profileId))
			where = where.AndAlso(device => device.ProfileId == profileId);
