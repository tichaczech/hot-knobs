...
public abstract class EntityListQueryHandler<TEntityListQuery, TEntity, TRepresentation> : EntityQueryHandler<TEntity>, IEntityListQueryHandler<TEntityListQuery, TEntity, TRepresentation>
	...
	where TRepresentation : class, IModel
{
	...
	public async ValueTask<ListResponse<TRepresentation>> Handle(TEntityListQuery query, CancellationToken cancellationToken)
	{
...
#pragma warning restore IDE0100 // Remove redundant equality
		...
		Expression<Func<TEntity, TRepresentation>> selector = entity => Mapper.Map<TRepresentation>(entity);
		if (query.Selector != default)
			selector = query.Selector;

		var items = Repository.FindAll(where, selector, query.PagingToken, query.SynchronizationToken, cancellationToken);
		return new ListResponse<TRepresentation>(items, "new-paging-token-here", "new-sync-token-here");
